...
class fetchonlyhiddennodeslistFilter
{
    // Return help text for this filter
    function getHelpText()
    {
        return '
--filter="fetchonlyhiddennodeslist;parent=<parent>[;classname=<class id list>][;limit=<limit>][;offset=<offset>][;depth=<depth>][;ignore_visibility]"
...
';
    ...
    function getObjectList()
    {
		$parameters = array( 'parent_node_id' => $this->parent_node_id,
						...
						'attribute_filter'=>array(array('visibility',0))
                        );
        if ( $this->limit > 0 )
